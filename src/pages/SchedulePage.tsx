    ...
    >
        ...
        <div className="p-6">
          {currentRound && currentRound.speakers.length > 0 ? (
                ...
                {currentRound.speakers.map((speaker, i) => (
                  <AnimatedListItem key={speaker.id} id={speaker.id} index={i}>
                    <div className="flex items-center gap-4 p-4 rounded-xl bg-zinc-800/30 border border-zinc-800/50 hover:border-zinc-700/50 transition-all">
                      <div className="flex items-center justify-center w-8 h-8 rounded-full bg-indigo-500/10 text-indigo-400 font-bold text-sm">
                        {i + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-zinc-500 truncate mt-0.5">{speaker.topic}</p>
                      </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-zinc-500 mb-4">本日尚未安排分享者</p>
      ...
      {nextQueue.length > 0 && (
        <AnimatedCard hoverable={false} delay={0.2}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold text-zinc-300">下一轮排队</h3>
          </div>
      ...
      {completedRounds.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider">历史记录</h3>
          {completedRounds.map((round, i) => (
            <AnimatedCard key={round.date} delay={0.1 * i} hoverable={false} className="!p-4">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-zinc-300">{round.date}</span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400">已完成</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {round.speakers.map(s => (
                  <span key={s.id} className="text-xs px-2.5 py-1 rounded-lg bg-zinc-800/80 text-zinc-400">
                    {s.name}: {s.topic}
                  </span>
                ))}
              </div>
